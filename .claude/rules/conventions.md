...
- **`users.ai_processing_consent_at`** (migrácia `0041`) je **zdroj pravdy pre súhlas s AI/OCR**. `ensureUser` ho načíta do `req.aiConsentGranted` a AI routy ho podávajú do `privacyGuard` ako `aiProcessingConsent: req.aiConsentGranted === true`. **Nikdy neber súhlas z tela requestu** — klient by si ho určoval sám. NULL = neudelený (fail-closed). Zmena súhlasu volá `invalidateUserCache`, inak by odvolanie platilo až po vypršaní 60 s cache.
